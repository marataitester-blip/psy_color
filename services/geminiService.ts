...
// In a real production app, ensure API keys are not exposed to the client.
...
export const analyzeText = async (userInput: string, language: Language): Promise<TarotAnalysis> => {
  try {
    ...
    throw new Error("Failed to analyze the spirit path.");
  }
};
...
export const generateTarotImage = async (prompt: string): Promise<string> => {
  try {
    ...
    throw new Error("Failed to manifest the vision.");
  }
};